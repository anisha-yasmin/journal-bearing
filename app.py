...
def solve_reynolds_1d(R, L, c, speed_rpm, viscosity_pas, epsilon, mesimport streamlit as st
import numpy as np
import plotly.graph_objects as go
...
if st.sidebar.button("Execute Hydrodynamic Solver", use_container_width=True):
    ...
    st.info("Adjust operating limits in the left panel and click execute to run the finite difference model loops.")h_pts=180):
    omega = 2 * np.pi * (speed_rpm / 60.0)
    theta = np.linspace(0, 2 * np.pi, mesh_pts)
    dtheta = 2.0 * np.pi / (mesh_pts - 1)
